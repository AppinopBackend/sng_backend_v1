...
const { generateReferCode, generate_otp, check_type } = require('../utils/Utils')
...
module.exports = {
    ...
    dashboard: async (req, res) => {
        try {
            ...
            let directUsersWithStaking = await Promise.all(direct.map(async (ref) => {
                ...
                let stackingAmount = stakingData.length > 0 ? stakingData[0].totalStackQuantity : 0;
            ...
            let countGreaterThanSelfTopup = directUsersWithStaking.reduce((count, ref) => {
        ...
        try {
            ...
            if (amount < 10 || amount % 5 !== 0) {
                return res.status(400).json({
                    success: false,
                    message: "You cannot transfer below $10 & the amount should be a multiple of $5",
                    data: []
                });
            }
            ...
            if (amount > sender_wallet_balance) {
                ...
                await Wallet.updateOne({ user_id: user_id }, { $set: { usdt_balance: sender_wallet_balance } }, { new: true })

                //Update Receiver Wallet Balance
                await Wallet.updateOne({ user_id: receiver_id }, { $set: { usdt_balance: receiver_wallet_balance } }, { new: true })
                ...
                let updated_sender_wallet = await Wallet.findOne({ user_id: user_id }).select('usdt_balance');
                let updated_receiver_wallet = await Wallet.findOne({ user_id: receiver_id }).select('usdt_balance');
                console.log(updated_sender_wallet, ": Updated Sender Wallet Balance")
                console.log(updated_receiver_wallet, ": Updated Receiver Wallet Balance")
