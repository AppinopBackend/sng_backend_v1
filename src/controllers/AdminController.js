...
module.exports = {
  login: async (req, res) => {
    try {
      ...
      if (user === null) {
        return res
          .status(500)
          .json({ success: false, message: "Invalid Admin User!!", data: [] });
      ...
      if (!compare) {
        ...
        let token = await JWT.generate_token(data, JWT_SECRET, JWT_EXPIRY_TIME);
        data.token = token;
        return res
          .status(200)
  ...
  sendOtp: async (req, res) => {
    try {
      ...
      // Optionally, check if admin exists for registration, etc.
      // let admin = await Admin.findOne({ email: email });
      // if (type === 'registration' && admin) {
      //     return res.status(406).json({ success: false, message: 'Admin already registered with this email', data: [] });
      // }

      // generate otp
      let otp = await require("../utils/Utils").generate_otp();
      console.log(otp, " : otp for Admin");
      // send otp to email
      await require("../utils/Marketing").sendVerificationCode(email, otp);
      // update otp in DB
  ...
  forgotPasswordAdmin: async (req, res) => {
    try {
      ...
      const check_otp = await Otps.findOne({ email_or_phone: email });
      ...
      const encryptedPassword = await Bcrypt.passwordEncryption(new_password);
      ...
      await Otps.deleteOne({ email_or_phone: email });
  ...
  changePasswordAdmin: async (req, res) => {
    try {
      ...
      // Get admin ID or email from req.user (set by authentication middleware)
      ...
      // Verify old password
      const isMatch = await Bcrypt.passwordComparison(
        old_password,
        admin.password
      );
      if (!isMatch) {
  ...
  updateSubadmin: async (req, res) => {
    try {
      ...
      if (update_data) {
        return res.status(200).json({
          success: true,
          message: "Sub admin updatted successfully",
          data: update_data,
        });
      } else {
        return res.status(200).json({
          success: false,
          message: "Some error occured while updating sub admin",
          data: [],
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (!status) {
        ...
        let depositData = await WalletTransaction.find({
          $and: [{ type: "DEPOSIT" }, { status: status }],
        });
        const userIds = depositData.map((item) => item.user_id);
        const users = await Users.find({ user_id: { $in: userIds } });
  ...
  updateLoginStatus: async (req, res) => {
    try {
      ...
      if (data.modifiedCount > 0) {
        return res.status(200).json({
          success: true,
          message: `Users status set to ${status}`,
          data: [],
        });
      }
    } catch (error) {
  ...
  getUserDetails: async (req, res) => {
    try {
      ...
      let totalStakingAmount = staking.reduce(
        (total, stake) => total + stake.amount,
        0
      );
      console.log(totalStakingAmount, "STACKING KA TOTAL");
      // Users deposit request
      let deposit = await Transaction.find({
        $and: [{ user_id: user_id }, { transaction_type: "DEPOSIT" }],
      });

      // Users withdraw request
      let withdraw = await Transaction.find({
        $and: [{ user_id: user_id }, { transaction_type: "WITHDRAWAL" }],
      });
      ...
      return res
        .status(200)
        .json({ success: true, message: "User details fetched!!", data: data });
    } catch (error) {
      return res
        .status(500)
        .json({ success: false, message: error.message, data: [] });
  ...
  transactionByType: async (req, res) => {
    try {
      ...
      let data = await WalletTransaction.find({ transaction_type: type })
        .sort({ createdAt: 1 })
        .skip(skip || 0)
        .limit(limit || 10);
      return res.status(200).json({
  ...
  fetchAllWithdrawals: async (req, res) => {
    try {
      ...
      if (!status) {
        ...
        console.log("inside else");
        let depositData = await WalletTransaction.find({
          $and: [{ type: "WITHDRAWAL" }, { status: status }],
        });
        ...
        return res.status(200).json({
          success: true,
          message: `Withdrawals with status ${status} Fetched Successfully`,
          data: depositData,
        });
      }
  ...
  dashboard: async (req, res) => {
    try {
      ...
      obj.totalDeposit = await WalletTransaction.countDocuments({
        type: "DEPOSIT",
      });
      obj.totalWithdrawal = await WalletTransaction.countDocuments({
        type: "WITHDRAWAL",
        status: "COMPLETED",
      });
      ...
      obj.totalStakingAmount =
        business.length > 0 ? business[0].totalStackQuantity : 0;

      // Total active users (users with staking_status = 'ACTIVE')
      obj.totalActiveUsers = await Users.countDocuments({
        staking_status: "ACTIVE",
      });

      // Total deposit amount
      let totalDepositTransactions = await WalletTransaction.find({
        type: "DEPOSIT",
      });
      let totalDepositAmount = totalDepositTransactions.reduce(
      ...
      let totalWithdrawal = await WalletTransaction.find({
        $and: [{ type: "WITHDRAWAL" }, { status: "COMPLETED" }],
      });
      let sumForTotalWithdrawal = totalWithdrawal.reduce(
        (sum, transaction) => sum + transaction.amount,
        0
      );
      ...
      let rewardTransactions = await Transaction.find({
        income_type: "sng_rewards",
      });
      let uniqueRewardUsers = [
        ...new Set(
          rewardTransactions.map((transaction) => transaction.user_id)
        ),
      ];
      let totalRewardAmount = rewardTransactions.reduce(
        (sum, transaction) => sum + transaction.amount,
        0
      );
  ...
  allUsersPackageList: async (req, res) => {
    try {
      ...
      return res.status(200).json({
        success: true,
        message: "All Staking Transaction Fetched!!",
        data: data,
      });
    } catch (error) {
      return res
    ...
    try {
      ...
      if (add_funds) {
        ...
        return res.status(200).json({
          ...
          data: funds2,
          transferDetails: adminTransfer,
        });
      } else {
        return res
          .status(400)
          .json({ success: false, message: "Failed to add funds", data: [] });
      }
    } catch (error) {
      return res
        .status(500)
  ...
  fundTransferHistory: async (req, res) => {
    try {
      ...
      return res.status(200).json({
        success: true,
        message: "Admin Fund Transfer History Fetched",
        data: tranferHistory,
      });
    } catch (error) {
      return res
  ...
  addUserStacking: async (req, res) => {
    try {
      ...
      if (
        !currentRank ||
        rankOrder.indexOf(rank) > rankOrder.indexOf(currentRank)
      ) {
  ...
  deductUserStacking: async (req, res) => {
    try {
      ...
      let user = await Users.findOne({ user_id: user_id }).sort({
        createdAt: -1,
      });
      if (!user) {
        return res.status(404).json({
      ...
      let stakes = await Staking.find({ user_id: user_id }).sort({
        createdAt: -1,
      });
      if (!stakes.length) {
        return res.status(404).json({
      ...
      let totalStakedAmount = stakes.reduce(
        (total, stake) => total + stake.amount,
        0
      );
      ...
      if (amount > totalStakedAmount) {
        return res.status(400).json({
          success: false,
          message:
            "Cannot Deduct! Deduction amount exceeds the total stacked amount",
          data: {
  ...
  adminStakingTransferHistory: async (req, res) => {
    try {
      let tranferHistory = await Staking.find({
        $or: [{ type: "ADMIN_DEDUCT_STAKING" }, { type: "ADMIN_STAKING" }],
      });
      // adding user name
      const userIds = tranferHistory.map((item) => item.user_id);
      const users = await Users.find({ user_id: { $in: userIds } });
  ...
  delete_notification: async (req, res) => {
    try {
      ...
      let delete_notification = await Notification.findByIdAndDelete({
        _id: id,
      });
  ...
  add_classic_news: async (req, res) => {
    try {
      ...
      const news_image =
        req.file != undefined ? `uploads/${req.file.filename}` : " ";

      let add_classic_news = await ClassicNews.create({
        title,
        description,
        news_image,
      });
      return res.status(200).json({
        success: true,
        message: "Classic News Sent Successfully!",
        data: add_classic_news,
      });
    } catch (error) {
      return res
        .status(500)
  ...
  classic_news_list: async (req, res) => {
    try {
      ...
      return res.status(200).json({
        success: true,
        message: "Classic News List Fetched",
        data: classic_news_list,
      });
    } catch (error) {
      return res
        .status(500)
  ...
  delete_classic_news: async (req, res) => {
    try {
      const { id } = req.params;
      let delete_classic_news = await ClassicNews.findByIdAndDelete({
        _id: id,
      });

      return res.status(200).json({
        success: true,
        message: "Classic News Deleted Successfully",
        data: delete_classic_news,
      });
    } catch (error) {
      return res
        .status(500)
  ...
  all_user_backup: async (req, res) => {
    try {
      ...
      const all_users = await Users.find(
        { staking_status: "ACTIVE" },
        "user_id name email phone activation_date createdAt"
      ).lean();
      ...
      return res
        .status(200)
        .json({ success: true, message: "User Data Backup Fetched", data });
    } catch (error) {
      return res
        .status(500)
        .json({ success: false, message: error.message, data: [] });
