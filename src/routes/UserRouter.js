...
const { support_upload } = require("../utils/Imageupload");
...
router
  .get("/", async (req, res, next) => {
    return res
      .status(200)
      .send({ success: true, welcome: "Welcome to Spark Net Global" });
  })
  ...
  .post(
    "/v1/user/user-fund-transfer",
    [token_verification],
    UserController.userToUserFundTransfer
  )

  // User Fund Transfer History By Admin
  .get(
    "/v1/admin/fund-transfer-history-by-admin/:id",
    UserController.UserFundTransferHistoryByAdmin
  )

  // Find User Profile Details By UserId
  .get(
    "/v1/user/user-profile/:user_id",
    [token_verification],
  ...
  // User to User Fund Transfer
  .get(
    "/v1/user/user-to-user-transfer-history",
    [token_verification],
    UserController.UserFundTransferHistory
  )
  ...
  .post(
    "/v1/user/sendSupport",
    [token_verification],
    support_upload.single("file"),
    UserController.sendSupportEmail
  );
